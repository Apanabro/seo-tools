...
const MetaGenerator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.generateMeta({ title, description, keywords, url });
      if (response.success) {
        setResult(response.data);
      } else {
        setError(response.error || 'Failed to generate meta tags');
      }
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="tool-form">
          <div className="form-group">
            ...
            <input
              type="text"
              id="title"
              placeholder="e.g., Best SEO Tools - Free Online Analysis"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              required
              maxLength={70}
            />
            <small style={{ color: title.length > 60 ? 'var(--google-red)' : 'var(--gray-500)' }}>
          ...
          <div className="form-group">
            ...
            <textarea
              id="description"
              placeholder="e.g., Professional SEO tools to analyze, audit, and optimize your website..."
              value={description}
              ...
              maxLength={170}
            />
            <small style={{ color: description.length > 160 ? 'var(--google-red)' : 'var(--gray-500)' }}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input
                type="text"
                id="keywords"
                placeholder="seo, tools, analysis, audit"
                value={keywords}
            ...
            {result.warnings && result.warnings.length > 0 && (
              <div style={{ marginBottom: '16px', padding: '12px', background: '#fff3cd', borderRadius: 'var(--radius-sm)' }}>
                {result.warnings.map((w, i) => (
                  <p key={i} style={{ color: '#856404', fontSize: '13px', margin: '4px 0' }}>Warning: {w}</p>
                ))}
              </div>
            )}
            <pre>{result.html}</pre>
